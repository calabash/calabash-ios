...
A brief explanation of demo. In the NavBar app an iOS Table view with a navigation bar is the main view. This uses the classing navigation controller 'push' animation. A common problem is that a view is considered visible by iOS before the animation completes. So often when you tap a table cell, and then wait for a button on the pushed view, the tap will fail (without warning) because the button is visible but animating. `wait_tap`, tries to solve this.
